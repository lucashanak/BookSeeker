"""Authentication & user management — recycled from MusicSeeker."""
import hashlib
import hmac
import json
import os
...
def _create_token(username: str, is_admin: bool) -> str:
    payload = json.dumps({"sub": username, "admin": is_admin, "exp": int(time.time()) + TOKEN_EXPIRY})
...
def init_admin(username: str, password: str):
    users = _load_users()
    if not users:
        users[username] = {"password": _hash_password(password), "is_admin": True}
        _save_users(users)
...
def get_current_user(request: Request) -> dict:
    ...
    return {"username": payload["sub"], "is_admin": users[payload["sub"]].get("is_admin", False)}
...
def create_user(username: str, password: str, is_admin: bool = False) -> bool:
    ...
    if username in users:
        return False
    users[username] = {"password": _hash_password(password), "is_admin": is_admin}
    _save_users(users)
    return True
